import type { IMembers } from '../types/members-type'

export const members: IMembers[] = [
  {
    id: 1,
    name: 'María José Guzman Rojas',
    degree: 'Comunicación Social y Periodismo',
    email: 'maria.guzman10@uac.edu.co',
    avatar: 'https://res.cloudinary.com/di85cn64g/image/upload/v1710816510/a77e77a8-985d-47bf-a6c9-87cc3676f4e4_zk9apu.jpg'
  },
  {
    id: 2,
    name: 'Kiara Cervantes Barrios',
    degree: 'Comunicación Social y Periodismo',
    email: 'kiara.cervantes@uac.edu.co',
    avatar: 'https://res.cloudinary.com/di85cn64g/image/upload/v1710815908/2fbffab4-a364-4005-8fd4-e32765ddb2cf_tdmoon.jpg'
  },
  {
    id: 3,
    name: 'Gianella Matonas',
    degree: 'Comunicación Social y Periodismo',
    email: 'gianella.matonas@edu.edu.co',
    avatar: 'https://res.cloudinary.com/di85cn64g/image/upload/v1710815900/d163d372-fcb6-42a1-b5a4-dfd7adf3b5af_g8bvpl.jpg'
  },
  {
    id: 4,
    name: 'Roberto',
    degree: 'Comunicación Social y Periodismo',
    email: 'maria.guzman10@uac.edu.co',
    avatar: 'https://res.cloudinary.com/di85cn64g/image/upload/v1710819409/d92018b4-806b-43e7-b9d2-62eb1e97419e_vqpzqd.jpg'
  }
]
